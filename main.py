...
# Объект бота
# Bot object
bot = Bot(token="6566679747:AAFARTP_fowsEIbv78X8queeVIN1OfqGesg")
# Диспетчер
#Dispatcher
dp = Dispatcher(bot)
sql.init_db()
...
#Константы
#Constants
id_admins = [6299670083]#ID админов (ID admins)
koef_ref = 0.01#ставка по реферальной программе для клиента (coef refferal programm for client)
partner_ref = 0.05#ставка по реферальной программе для партнера (coef refferal programm for partners)
komission_casino = (1 - 0.07)#комиссия в играх между игроками(казино не участвует) (casino commision)
channel_of_requests_withdraw = -1002241499402#канал, куда приходят запросы на вывод средств (channel of requests to withdraw)
channel_of_deposit = -1002241499402#канал, куда приходят уведомления о депозите(channel with notifications of deposits)
channel_of_error = -1002241499402#канал, куда приходят логи с ошибками(channel with logs of errors)
id_chat_bot = -1002191163363 #чат бота(chat bot)
chat_id_group = -1002241499402 #канал бота(channel bot)
game_of_id = {
    14: 'Bowling',
    15: 'Penalty',
    16: 'Darts',
...
#exit from unstarted multigame выход из неначавшейся мультиигры
...
#exit from started multigame выход из начавшейся мультиигры
